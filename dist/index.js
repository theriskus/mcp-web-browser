...
function randomDelay(minMs = 300, maxMs = 1500) {
...
function httpGet(url, delay = true) {
    if (delay)
        void randomDelay();
    return new Promise((resolve, reject) => {
        https.get(url, { headers: randomHeaders() }, (res) => {
...
let browser = null;
function randomPlaywrightHeaders() {
    const chromeVersions = ["125", "126", "127", "128", "129", "130", "131"];
    const chromeVer = pick(chromeVersions);
    const platforms = [
...
async function newPageWithRandomHeaders(browser) {
...
server.tool("web_search", "Search the web and return results with titles, URLs, and snippets.", {
...
server.tool("web_fetch", "Fetch a URL and return its parsed HTML content as text, optionally with extracted links.", {
...
server.tool("web_scrape", "Use Playwright (browser) to load a page with JavaScript support and extract content. Best for dynamic/SPA sites.", {
...
server.tool("open_url", "Open a URL in a headless browser and return the page title, HTTP status, and a short content preview.", {
    ...
    try {
        ...
        await page?.close().catch(() => { });
        return {
            content: [{ type: "text", text: `Open URL failed: ${err.message}` }],
            isError: true,
        };
    }
});
